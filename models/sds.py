...
class StableDiffusion(nn.Module):
    ...
    def __init__(self, device, version):
        ...
        self.vae = AutoencoderKL.from_pretrained(self.model_key, subfolder="vae",use_safetensors=True).to(self.device)

        # 2. Load the tokenizer and text encoder to tokenize and encode the text. 
        self.tokenizer = CLIPTokenizer.from_pretrained(self.model_key, subfolder = "tokenizer")
        self.text_encoder = CLIPTextModel.from_pretrained(self.model_key, subfolder = "text_encoder",use_safetensors=True).to(self.device)

        # 3. The UNet model for generating the latents.
        self.unet = UNet2DConditionModel.from_pretrained(self.model_key, subfolder="unet",use_safetensors=True).to(self.device)
    ...
    def manual_backward_nfsd(self, text_embeddings, pred_rgb:torch.Tensor, guidance_scale=100, pred_depth:torch.Tensor = None, latent=None) -> None:
        """
        backward the SDS loss the the pred_rgb.
        Input:
            pred_rgb: Tensor, [1, 3, H, W] assume requires grad
        ...
        with torch.no_grad():
            ...
            noise_pred_neg = self.unet(latent_model_input, t, encoder_hidden_states=text_neg_embeddings).sample
        ...
        # CFG
        if t > 200:
            noise_pred = (noise_pred_uncond - noise_pred_neg_text) + guidance_scale * (noise_pred_text - noise_pred_uncond)
        else:
            noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_text - noise_pred_uncond)
    ...
    def manual_backward_csd(self, text_embeddings, pred_rgb:torch.Tensor, guidance_scale=100, pred_depth:torch.Tensor = None, latent=None) -> None:
        """
        backward the SDS loss the the pred_rgb.
        Input:
            pred_rgb: Tensor, [1, 3, H, W] assume requires grad
    ...
    def prompt_to_img(self, prompts, height=512, width=512, num_inference_steps=50, guidance_scale=7.5, depth = None, latents=None):
